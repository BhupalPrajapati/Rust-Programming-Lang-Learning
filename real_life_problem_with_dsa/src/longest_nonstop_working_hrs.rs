...
fn longest_time_slot(working_slot: Vec<Vec<u8>>) -> usize {
    ...
    for (i, &period) in employee_longest_nonstop_time.iter().enumerate() {
        println!("Employee Number {} has worked nonstop for {} slots", i, period);
    }

    let max_val = employee_longest_nonstop_time.iter().max().unwrap();

    employee_longest_nonstop_time.iter().position(|&x| x == *max_val).unwrap()
}
 ...
 fn main() {
    ...
    let schedule:  Vec<Vec<u8>> = vec![vec![4,1,2,5,6,10,11],
 ...
 println!("Employees Number {} has the highest number of non-stop working slots",longest_time_slot(schedule));   
}